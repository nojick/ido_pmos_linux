...
static void __maybe_unused yas_work_func(struct work_struct *work)
{
	struct yas_state *st
	...
	int32_t poll_delay;
	uint32_t time_before, __maybe_unused time_after;
	int ret, i;
	ktime_t timestamp;
	...
	if (ret == 1) {
		...
		input_event(st->input_dev, EV_SYN, SYN_TIME_SEC, ktime_to_timespec64(timestamp).tv_sec);
		input_event(st->input_dev, EV_SYN, SYN_TIME_NSEC, ktime_to_timespec64(timestamp).tv_nsec);
...
static void yas_remove(struct i2c_client *i2c)
{
